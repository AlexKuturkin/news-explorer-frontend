...
export function validate(element) {
    ...
    if (element.value.length == 0) {
        ...
        return false;
    } else if (element.type === 'email' && element.value.match(/^\w+([\.\-\]?\w+)@\w+([\.\-]?\w+)*(\.\w{2,8})$"*/) === null) {
        errorElement.textContent = "Email в формате: sega@yandex.ru";
        element.parentNode.classList.add("popup__content_invalid");
        return false;
    }
    else if (element.type === 'password' && element.value.length < 10) {
        errorElement.textContent = "Пароль должен быть от 10 и более символов";
        element.parentNode.classList.add("popup__content_invalid");
        return false;
    }
    else if (element.name === 'reg_name' && element.value.length < 2) {
        errorElement.textContent = "Имя должно быть от 2 и более символов";
        element.parentNode.classList.add("popup__content_invalid");
        return false;
    }
    return true;
